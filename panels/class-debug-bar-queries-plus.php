...
class Debug_Bar_Queries_Plus extends Debug_Bar_Panel {
	function init() {
		$this->title( __( 'Queries Plus', 'debug-bar' ) );
	}
	...
	function render() {
		...
		if ( ! empty( $wpdb->queries ) ) {
			...
			foreach ( $wpdb->queries as $q ) {
				...
				$out .= "<li class='dbqp-list-item' data-count={$counter} data-ms_elapsed=" . number_format( sprintf( '%0.1f', $elapsed * 1000 ), 1, '.', ',' ) . '>';
				$out .= "$query<br/>";
				$out .= "<div class='qdebug'>$debug <span>#{$counter} (" . number_format( sprintf( '%0.1f', $elapsed * 1000 ), 1, '.', ',' ) . 'ms)</span></div>';
				$out .= "</li>\n";
			}
			$out .= '</ol>';
		} else {
			if ( 0 == $wpdb->num_queries ) {
		...
		$heading .= '<h2><span>Sort By:</span>';
		$heading .= '<select name="dbqp-sort-by" id="dbqp-sort-by">';
		$heading .= '<option value="elapsed">Elapsed</option>';
		$heading .= '<option value="ordinal" selected="selected">Execution Order</option>';
		$heading .= '</select></h2>';
